document.addEventListener('DOMContentLoaded', () => {
    ...
    function showSynergies() {
        synergyResultsContainer.innerHTML = ''; // Clear current synergies
        let glowingCharacters = []; // Keep track of characters to glow
        ...
        handpickedSynergies.forEach(synergy => {
            ...
            if (selectedCharacters.includes(synergyCharacters[0]) && selectedCharacters.includes(synergyCharacters[1])) {
                ...
                // Add glowing characters to the array
                glowingCharacters = glowingCharacters.concat(synergyCharacters);
            }
        });

        // Apply the glow effect to the selected characters that are part of synergies
        glowingCharacters.forEach(character => {
            document.querySelector(`.character[data-character="${character}"]`).classList.add('glowing');
        });
    }

    // Helper function to clear glow effect (when clearing selections)
    function clearGlowEffect() {
        document.querySelectorAll('.character').forEach(character => {
            character.classList.remove('glowing');
        });
    }
});
